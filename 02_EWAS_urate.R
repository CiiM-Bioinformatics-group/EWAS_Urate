...
sum(uv1$id2 == paste0("X", rownames(mv1.t)))
sum(uv1$id2 == pdv1$id2)

RLMtest = function(meth_matrix, methcol, urate, age, gender, plate, CD8, CD4, NK, B, Mono, Neu) {mod = try(rlm(meth_matrix[, methcol] ~ urate+age+gender+plate+CD8+CD4+NK+B+Mono+Neu, maxit=200))
...
res <- mclapply(setNames(seq_len(ncol(mv1.t)), dimnames(mv1.t)[[2]]), RLMtest, meth_matrix=mv1.t, urate=uv1$con, age=pdv1$age, gender=pdv1$gender, plate=pdv1$Sample_Plate, CD8=pdv1$CD8T, CD4=pdv1$CD4T, NK=pdv1$NK, B=pdv1$Bcell, Mono=pdv1$Mono, Neu=pdv1$Neu)
...
pdf("output/v1_mval_trimmed_m4_qqplot_rm_plate11.pdf")
qq(result$P_VAL, main = "Q-Q plot of visit 1 model 4 (v1_mval_trimmed)")
dev.off()

write.xlsx(result, file = "output/v1_mval_trimmed_.rlm.xlsx")
